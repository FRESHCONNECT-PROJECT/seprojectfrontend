...
const GroceryView = () => {
  ...
  const fetchUserData = async (email) => {
    try {
      ...
      fetchGroceries(university); // Fetch groceries using the university name
    } catch (err) {
  ...
  const fetchGroceries = async (university) => {
    try {
      ...
      const allGroceries = response.data.groceries;
  ...
  if (error) {
    return <p className="text-red-500">{error}</p>; // Display the error if any
  }

  return (
    <div className="container mx-auto p-6 mt-20">
      <h1 className="text-2xl font-bold text-center mb-6">
        Groceries Near {university}
      </h1>
