...
const socket = io('https://chatapis.fresh-connect.online');
...
const GroupChat = () => {
  ...
  const fetchMessagesForRoom = async (roomName) => {
    try {
      ...
      // Include userDetails for old messages if not already included
      const messagesWithUserDetails = response.data.map((message) => ({
        ...message,
        userDetails: message.userDetails || userDetails, // Fallback to current user details if not available
      }));
  ...
  const handleSendMessage = () => {
    if (newMessage.trim() && selectedRoom) {
      const messageData = {
        ...
        userDetails, // Include user details when sending message
        profilePics :userDetails.profilePic
      };
  ...
  const renderMessage = (message, isMyMessage) => {
    // Check if the message has a user profile picture, prioritizing the new one.
    const profilePic = message.ProfilePics || 
